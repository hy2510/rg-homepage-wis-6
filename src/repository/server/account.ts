...
async function signin(
  customer: string,
  input: { id: string; password: string; ipv4: string; deviceType: string },
) {
  const request = makeRequest({
    customer,
    path: getPath('signin'),
...
async function staffMe(token: string) {
  const request = makeRequest({
    token,
    path: getPath('staff-me'),
    option: {
      method: 'get',
    },
  })
...
async function mriage(token: string, input: { uid: string }) {
  const request = makeRequest({
    token,
    path: getPath('mirage'),
    option: {
