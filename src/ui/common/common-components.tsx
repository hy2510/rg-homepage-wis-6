    ...
    <div className={`${style.progress_bar} ${slim && style.slim}`}>
      ...
      {check && (
        <div className={style.check} style={{ width: check }}>
          <div
            className={`${style.tool_tip} ${onClickToolTip && style.click}`}
            onClick={onClickToolTip}>
            {toolTip}
            {onClickToolTip && <span className={style.arrow_icon}></span>}
          </div>
        </div>
      )}
    </div>
...
export const NoticeBoardContainer = ({
  ...
  const styleGrid = grid
    ? style.notice_board_container_grid
    : style.notice_board_container
